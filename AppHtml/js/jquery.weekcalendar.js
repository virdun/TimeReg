...
(function($) {
    ...
    $.widget("ui.weekCalendar", {
        ...
        removeUnsavedEvents : function() {
            //this.element.find(".new-cal-event").css('border', '3px solid #333333');
        ...
        updateEvent : function (calEvent) {
            //alert('updateEvent ' + calEvent.customer_name);
        ...
        _renderEvents : function (events, $weekDayColumns) {
            ...
            $.each(eventsToRender, function(i, calEvent){
                ...
                //alert('render: ' + $weekDay + ' ' + new Date(calEvent.end.getTime()));
        ...
        _renderEvent: function (calEvent, $weekDay) {
            ...
            var eventClass, eventHtml, $calEvent, $modifiedEvent, style;
            //alert(calEvent.id);
            eventClass = calEvent.id ? "cal-event" : "cal-event new-cal-event";
            //alert(calEvent.user_color);
            if (jQuery.trim(calEvent.user_color) == '') {
                style = ' style="background: #999999; color: #000000;" ';
            } else {
                style = ' style="background: '+calEvent.user_color+'; color: #000000;" ';
            }
            eventHtml = "<div class=\"" + eventClass + " ui-corner-all\">\
                <input type=\"hidden\" name=\"id\" value=\"" + calEvent.id + "\">\
                <div class=\"time ui-corner-all\" " + style + "></div>\
                <div>\
                <div class=\"user_name\" style=\"display: inline;\"></div>\
                <div class=\"customer_name\" style=\"display: inline;\"></div>\
                </div>\
                <div class=\"title\"></div>\
                <div class=\"body\"></div>\
                </div>";
        ...
        _findWeekDayForEvent : function(calEvent, $weekDayColumns) {
            ...
            var $weekDay;
            $weekDayColumns.each(function(){
                if (new Date(calEvent.start.getTime()) == 'Invalid Date') {
                    alert('Invalid date in _findWeekDayForEvent.'  + calEvent.start );
                }
                //alert('Date: ' + calEvent.start ) ;
                //alert('if ' + new Date($(this).data("startDate").getTime()) + '<= ' + new Date(calEvent.start.getTime()) +' && '+ new Date($(this).data("endDate").getTime())+' >= '+new Date(calEvent.end.getTime()));
        ...
        _updateEventInCalendar : function (calEvent) {
            //alert('_updateEventInCalendar' + calEvent.title);
            var self = this;
            var options = this.options;
        ...
        _refreshEventDetails : function(calEvent, $calEvent) {
            var self = this;
            var options = this.options;
            $calEvent.find(".id").text(calEvent.id);
            ...
            $calEvent.find(".user_color").text(calEvent.user_color);
            $calEvent.find(".user_name").text(calEvent.user_name);
            $calEvent.find(".customer_name").text(calEvent.customer_name);
            //www.google.sealert(calEvent.body + ' = ' + calEvent.text);
            if (calEvent.body == undefined) {
                calEvent.body = calEvent.text;
            } else {
                calEvent.text = calEvent.body;
            }
            $calEvent.find(".body").text(calEvent.text);
