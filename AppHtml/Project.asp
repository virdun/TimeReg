...
<script type="text/javascript">
    ...
    function displayEditDialog(bClear){
        ...
        oSelectForm = vdf.getForm("project_select");
        oPopupForm = vdf.getForm("project_popup");
        
        //  display the dialog
        var oDialog = new vdf.gui.ModalDialog();
        oDialog.sTitle = "Edit project";
...
<% End If %>
        ...
        if(bClear){
            ...
            oPopupForm.doFindByRowId("project", oSelectForm.getBufferValue("project.rowid"));
        }
    ...
    function onDialogButtonClick(oEvent){
        switch(oEvent.sButtonName){
            case "close":
                ...
                break;
            case "save":
                vdf.getForm("project_popup").doSave();
                break;
        }
    }
    
    //
    //  Handles the onAfterSave event of the project DD of the popup form and 
    ...
    function onAfterPopupSave(oEvent){
        ...
        if(!oEvent.bError){
            ...
            oSelectForm.doFindByRowId("project", oPopupForm.getBufferValue("project.rowid"));
            vdf.gui.closeModalDialog();
        }
    }
    ...
    //
    //  Initialization method of the project_select form which is called after
    //  the form has initialized. It attaches the onListEnter function to the
    //  onEnter event to the project_lookup control.
    //  
    //  Params:
    //      oForm   Reference to the form object.
    //
    function initSelectForm(oForm){
        oForm.getControl("project_lookup").onEnter.addListener(onListEnter);
    }

    //
    //  Initialization method of the project_popup form which is called after 
    ...
    function initPopupForm(oForm){
        ...
        //  Attach the onAfterPopupSave function to the onAfterSave event of the project DD
        oForm.getDD("project").onAfterSave.addListener(onAfterPopupSave);
    }
</script>
...
<form action="none" name="form1" autocomplete="off" vdfControlType="form" vdfControlName="project_select" vdfInitializer="initSelectForm" vdfMainTable="project" vdfServerTable="project" vdfWebObject="oProject">
...
<table width="100%">
    <tr>
        <td>
            <% if (bEditRights) then %>
                <h3>Project Entry and Maintenance</h3>
            <% else %>
                <h3>Project Query</h3>
            <% end if %>
        </td>
        ...
        <td>
            ...
            <table class="Toolbar" >
                ...
                <% If (bEditRights) Then %>                
                    ...
                    <% If (oSessionManager.call("get_HasRights", sSessionKey, "delete", "oProject", "")) Then %>
                    <td class="ButtonCell"><input type="button" class="ButtonNormal" name="delete" value="Delete" onclick="vdf.core.findForm(this).doDelete('project');" title="Delete (Shift - F2)" /></td>
                    <% End If %>
                <% End If %>
                    <td>&nbsp;</td>
                </tr>
    ...
    <tr>
        <td>
            <table style="width: 100%" vdfControlType="lookup" vdfControlName="project_lookup" vdfMainTable="project" vdfRowLength="10" vdfReturnCurrent="true" vdfFocus="true" vdfFixedColumnWidth="true">
                <tr vdfRowType="header">
                    <th vdfDataBinding="project__name">Projekt</th>
 ...
 <div id="hidden_popup" style="visibility: hidden">
    <form   action="none" 
            ...
            autocomplete="off" 
            vdfControlType="form" 
            vdfControlName="project_popup" 
            vdfInitializer="initPopupForm" 
            vdfMainTable="project" 
                        ...
                        <table class="EntryTable">
                            ...
                            <tr>
                                ...
                                <td>
                                    ...
                                    <input class="LookupButton" type="button" title="List of all records" tabindex="-1" vdfControlType="lookupdialog" vdfWebObject="oProject" vdfLookupTable="CUSTOMER" vdfLookupFields="customer__name, customer__customerid" />
                                </td>
                            </tr>
                        </table>
                    </div>
