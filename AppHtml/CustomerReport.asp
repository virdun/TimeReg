      ...
      <% If RunReport then '-------------run the report ------------- %>
	    ...
	    <table>
	      <tr>
		    ...
		    <% If EndRowId <>"" then %>
            <td style="width: 10px"><a class="ReportNavigation" href="CustomerReport.asp?RunReport=1&amp;Start=<%=EndRowId%>&amp;Index=<%=Index%>" title="Next page">>></a></td>
            <% end if %>
            <td style="width: 645px" align="right"><a class="ReportNavigation" href="CustomerReport.asp">New Report</a> </td>
		  </tr>
	    </table>   
      <% else  '-------------set up the report ------------- %>
        
        <form action="CustomerReport.asp" name="form1" autocomplete="off" method="GET" vdfControlType="form" vdfControlName="customer_form" vdfMainTable="customer" vdfServerTable="customer" vdfWebObject="oCustomer">        
          <blockquote>
            <table border="0">
              <tr>
			  ...
			  <tr>
			    ...
			    <td><% iResult = oCustomerReport.call("get_CreateFindIndexCombo", "Index", "Customer", Index) %></td>
			  </tr>
		    </table>
		  </blockquote>
          <p>
